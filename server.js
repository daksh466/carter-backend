...
const models = require('./backend/src/models/index.js');
const { mergeDuplicateSpareParts } = require('./backend/src/utils/sparePartDedup');
const { requireDestructiveAuth, requireDbConnected } = require('./backend/src/middlewares/securityGuards');
const {
  Store,
  Machine,
  SparePart,
...
console.log('Serving ERP API on port', PORT);
...
app.get('/api/health', (req, res) => {
  ...
  res.json({ 
    ...
    dbConnected,
    modelsStatus,
    endpoints: ['stores', 'transfers', 'purchase-orders', 'alerts', 'orders-list'],
...
app.post('/api/admin/migrate-variants', requireDbConnected, requireDestructiveAuth, async (req, res) => {
  const { migrateVariants } = require('./backend/src/utils/migrateVariants');
  ...
  try {
    ...
    if (err?.code === 11000) {
      try {
        ...
        const selectedMachineIds = parseMachineIdsFromBody(req.body);
        const rawPurchaseCost = req.body?.purchase_cost ?? req.body?.cost ?? req.body?.costPrice ?? req.body?.unitPrice ?? req.body?.price;
String(rawPurchaseCost).trim() === '' ? null : Number(rawPurchaseCost);
        if (normalizedPurchaseCost !== null && (Number.isNaN(normalizedPurchaseCost) || normalizedPurchaseCost < 0)) {
          return res.status(400).json({ success: false, message: 'purchase_cost must be a non-negative number' });
        }
        const existing = await SparePart.findOne({ normalized_name: normalizedName, store_id: storeId });
  ...
  app.use((req, res, next) => {
    if (req.method === 'GET' && !req.path.startsWith('/api')) {
      return res.status(404).json({
        success: false,
        message: 'Frontend served separately in dev (Vite).'
      });
    }
    next();
  });
...
if (require.main === module) {
  const server = app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ CarterA++ ERP API v2 running on port ${actualPort}`);
    console.log(`Health: http://localhost:${actualPort}/api/health`);
    console.log(`Test stores: http://localhost:${actualPort}/api/stores`);
  });

  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.log(`Port ${PORT} already in use. Kill with scripts/kill-ports.js`);
      process.exit(1);
    }
